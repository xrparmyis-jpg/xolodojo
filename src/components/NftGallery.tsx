...
const parseSocialsFromPreferences = (
  preferences: unknown
): PinnedNftSocials => {
  if (!preferences) {
    return {};
...
export default function NftGallery({
  ...
  const [resolvedNftThumbnails, setResolvedNftThumbnails] = useState<
    Record<string, string | null>
  >({});
  const [resolvedNftTitles, setResolvedNftTitles] = useState<
    Record<string, string | null>
  >({});
  const [resolvedNftCollections, setResolvedNftCollections] = useState<
    Record<string, string | null>
  >({});
  const [resolvedNftTraits, setResolvedNftTraits] = useState<
    Record<string, NftTrait[]>
  >({});
  const [collectionFallbackTokens, setCollectionFallbackTokens] = useState<
  ...
  /** False until the first pinned-NFT fetch for this wallet finishes (success or error). */
  const [pinnedNftsSettled, setPinnedNftsSettled] = useState(false);
  ...
  const [pendingUnpinTokenId, setPendingUnpinTokenId] = useState<string | null>(
    null
  );
  const [isPinActionLoading, setIsPinActionLoading] = useState(false);
  const [isSelectedNftImageLoaded, setIsSelectedNftImageLoaded] =
    useState(false);
  // Cache loaded NFT image URLs by tokenId
  const [loadedNftImageUrls, setLoadedNftImageUrls] = useState<
  ...
  const [availableProfileSocials, setAvailableProfileSocials] =
    useState<PinnedNftSocials>({});
  const [pinDraftSocials, setPinDraftSocials] = useState<ProfileSocials>({});
  const [pinVisibleSocialInputs, setPinVisibleSocialInputs] = useState(() =>
    createEmptyVisibleInputs()
  );
  const [pendingRemovePinSocial, setPendingRemovePinSocial] =
    useState<SocialPlatformKey | null>(null);
  const { showToast } = useToast();
  const metadataResultCacheRef = useRef<
    Partial<Record<string, NftMetadataResolution | null>>
  >({});
  const metadataRequestCacheRef = useRef<
    Partial<Record<string, Promise<NftMetadataResolution | null>>>
  >({});
  ...
  const getProxiedUrl = useCallback(
    (url: string, mode: 'json' | 'binary' = 'binary'): string => {
    ...
    return (
      candidates.find(
        candidate =>
          ...
          (isHttpUrl(candidate) && !isJsonUrl(candidate))
      ) || null
    );
  };

  const deriveCollectionNameFromTitle = (
    title: string | null
  ): string | null => {
    if (!title) {
      return null;
  ...
  const pickNftDetailsFromMetadata = (
    metadata: unknown
  ): NftMetadataResolution | null => {
    if (!metadata || typeof metadata !== 'object') {
      return null;
    }
    ...
    const collectionRecord = metadataRecord.collection as
    ...
    const propertiesRecord = metadataRecord.properties as
      | Record<string, unknown>
      | undefined;
    const collectionNameCandidates = [
      ...
      (propertiesRecord?.collection as Record<string, unknown> | undefined)
        ?.name,
    ];
    ...
    const collectionName =
      metadataCollectionName || deriveCollectionNameFromTitle(title);
  ...
  const resolveNftDetailsFromMetadata = useCallback(
    async (uri: string | null): Promise<NftMetadataResolution | null> => {
      if (!uri) {
        return null;
      }
      ...
      const resolutionPromise = (async () => {
        ...
        for (const metadataUrl of metadataUrls) {
          try {
            const response = await fetch(getProxiedUrl(metadataUrl, 'json'), {
              method: 'GET',
            });
  ...
  /** Titles for `?Xpin=` — include pin titles (globe edit links use pin.title, not NFT metadata). */
  const profilePinResolvePins = useMemo(() => {
    const byToken = new Map<string, { token_id: string; title?: string | null }>();
    for (const n of nfts) {
      byToken.set(normalizeNfTokenId(n.token_id), {
        token_id: n.token_id,
        title: lookupResolvedStringField(resolvedNftTitles, n.token_id),
      });
    }
    for (const p of pinnedNftItems) {
      const key = normalizeNfTokenId(p.token_id);
      const existing = byToken.get(key);
      const pinTitle = typeof p.title === 'string' ? p.title.trim() : '';
      byToken.set(key, {
        token_id: p.token_id,
        title: pinTitle || existing?.title || null,
      });
    }
    return Array.from(byToken.values());
  }, [nfts, resolvedNftTitles, pinnedNftItems]);
  ...
  const profilePinDeepLinkOpenedRef = useRef(false);

  useEffect(() => {
    profilePinMissingNotifiedRef.current = false;
    profilePinDeepLinkOpenedRef.current = false;
  }, [profilePinTokenParam, walletQueryForPinnedApi]);
  ...
  useEffect(() => {
    ...
    const loadProfileSocials = async () => {
      try {
        ...
        const profileSocials = parseSocialsFromPreferences(
  ...
  useEffect(() => {
      ...
      await Promise.all(
        paginatedNfts.map(async nft => {
          ...
          if (metadataDetails) {
            ...
            fallbackUpdates[nft.token_id] =
              metadataDetails.isCollectionFallback;
    ...
    debugNft,
    paginatedNfts,
    resolveNftDetailsFromMetadata,
    resolvedNftThumbnails,
  ]);
      ...
      selectedNftTokenId
        ? (nfts.find(
            nft =>
              ...
              normalizeNfTokenId(selectedNftTokenId)
          ) ?? null)
        : null,
  ...
  const selectedNftTraits = selectedNft
    ? resolvedNftTraits[selectedNft.token_id]
    : undefined;
  ...
  const selectedNftThumbnailSrc =
    selectedNft && loadedNftImageUrls[selectedNft.token_id]
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      setResolvedNftTraits(prev => ({
        ...prev,
        [selectedNft.token_id]: details.traits,
      }));
    })();
    return () => {
      cancelled = true;
    ...
    () =>
      pinTargetTokenId
        ? (nfts.find(
            nft =>
              normalizeNfTokenId(nft.token_id) ===
              normalizeNfTokenId(pinTargetTokenId)
          ) ?? null)
        : null,
  ...
  const pinTargetCollectionName = pinTargetNft
    ? lookupResolvedStringField(
        resolvedNftCollections,
        pinTargetNft.token_id
      ) ||
  ...
  const pinOpenSocialPlatforms = useMemo(
    () =>
      socialPlatformOrder.filter(
        platform => pinVisibleSocialInputs[platform.key]
      ),
  ...
  const handlePinSocialInputChange = (
  ...
  const openPinModalForCreate = (tokenId: string) => {
    ...
    setPinLocation(null);
    const defaultTitle =
      lookupResolvedStringField(resolvedNftTitles, tokenId) ??
  ...
  const openPinModalForEdit = useCallback(
    (tokenId: string) => {
      const idKey = normalizeNfTokenId(tokenId);
      const existing = pinnedNftItems.find(
        p => normalizeNfTokenId(p.token_id) === idKey
      );
      if (!existing) {
    ...
    [
      applyPinSocialsSeedWithOptionalPin,
      pinnedNftItems,
      resolvedNftTitles,
      setPinSocialsUiFromDraft,
      showToast,
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      if (
        pinnedTokenIdSet.has(normalizedTarget) &&
        !profilePinDeepLinkOpenedRef.current
      ) {
        profilePinDeepLinkOpenedRef.current = true;
        openPinModalForEdit(nftRow.token_id);
      }
      setSearchParams(
        prev => {
          const next = new URLSearchParams(prev);
  ...
  const handleSubmitPin = useCallback(async () => {
    ...
    const pinTargetThumbnailUrl = getNftThumbnailUrl(
      pinTargetNft.token_id,
      pinTargetNft.uri
    );
    try {
      setIsPinActionLoading(true);
      const socialsPayload = normalizeSocials(
        pinDraftSocials
      ) as PinnedNftSocials;
      syncPinSocialsToProfile?.(normalizeSocials(pinDraftSocials));
      ...
      showToast(
        'error',
        `Failed to pin NFT: ${error instanceof Error ? error.message : String(error)}`
      );
    } finally {
  ...
  const handleConfirmUnpin = async () => {
    ...
    try {
      ...
      showToast(
        'error',
        `Failed to unpin NFT: ${error instanceof Error ? error.message : String(error)}`
      );
    } finally {
      setIsPinActionLoading(false);
    }
        ...
        {paginatedNfts.map(nft => (
          ...
          >
            <div className="relative w-full max-w-[180px]">
              ...
              >
                {(() => {
                  let directCandidates = getDirectNftThumbnailCandidates(
                    nft.uri
                  ).filter(Boolean);
                  const isCollectionFallback =
                    collectionFallbackTokens[nft.token_id] === true;
                  const resolved = resolvedNftThumbnails[nft.token_id];
                  if (
                    (!directCandidates || directCandidates.length === 0) &&
                    resolved
                  ) {
                  ...
                  if (!directCandidates || directCandidates.length === 0) {
                    return (
                      <div className="relative flex aspect-square w-full items-center justify-center overflow-hidden rounded bg-white/5">
                        <FontAwesomeIcon
                          icon={faSpinner}
                          className="text-white/60 animate-spin text-2xl"
                        />
                      </div>
                    );
                  }
              ...
              {(() => {
                const isPinned = pinnedTokenIdSet.has(
                  normalizeNfTokenId(nft.token_id)
                );
      ...
      {totalNftPages > 1 && (
        <div className="mt-3 flex items-center justify-between text-xs text-white/70">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              onClick={() =>
                setCurrentNftPage(page => Math.min(totalNftPages, page + 1))
              }
            ...
            <div className="space-y-2 text-sm">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void handleCopyValue(
                        `${selectedNft.token_id}-collection`,
                        selectedNft.issuer
                      )
                    }
                    title={
              ...
              {selectedNft.uri ? (
                <div className="border-t border-white/10 pt-3">
                  ...
                  {selectedNftTraits === undefined ? (
                    ...
                    <p className="text-white/60 text-sm leading-relaxed">
                      ...
                      to the page URL to log keys and a JSON sample in the
                      console.
                    </p>
                  ) : (
                ...
                <p className="text-white/55 text-sm">
                  This token has no URI on ledger — trait metadata is
                  unavailable.
                </p>
              )}
            </div>
        ...
        title={
          pinSuccessState
            ? pinSuccessState.kind === 'updated'
              ...
              : 'Pin added to Xglobe'
            : pinFormMode === 'edit'
              ? 'Edit your Xglobe Xpin'
              : 'Choose your Xglobe Xpin location on the globe'
        }
              ...
              <p className="mt-1 text-white/75">
                ...
                {pinSuccessState.kind === 'updated' ? (
                  ...
                  <>
                    was added to{' '}
                    <span className="font-medium text-white">
                      {pinSuccessState.collectionName}
                    </span>
                    .
                  </>
                )}
              ...
              {pinFormStep === 1 ? (
                <div className="max-h-[min(72vh,640px)] space-y-4 overflow-y-auto overflow-x-visible md:overflow-x-hidden pr-1">
                  ...
                  <div className="w-full min-w-0 flex flex-col">
                    ...
                    <p className="text-xs text-white/55 mb-3">
                      Add handles to show on this pin&apos;s globe popup. They
                      are saved on the pin, not your password profile.
                      {syncPinSocialsToProfile
                      ...
                      {socialPlatformOrder.map(platform => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() =>
                              handlePinActivateSocial(platform.key)
                            }
                            className={`cursor-pointer relative inline-flex h-9 w-9 items-center justify-center rounded-full border transition-all duration-200 ${
                              isActive
                          ...
                          return (
                            ...
                            >
                              ...
                              <button
                                ...
                                onClick={() =>
                                  setPendingRemovePinSocial(platform.key)
                                }
                  ...
                  <div className="flex w-full flex-wrap items-center justify-end gap-3 pt-1">
                    {pinFormMode === 'edit' ? (
                      <Button
                        type="button"
                        onClick={() =>
                          setPendingUnpinTokenId(pinTargetNft.token_id)
                        }
                        disabled={isPinActionLoading}
                ...
                <div className="w-full min-w-0 flex flex-col overflow-visible">
                  ...
                  <MapBoxPinLocation
                    ...
                    markerImageUrl={getNftThumbnailUrl(
                      pinTargetNft.token_id,
                      pinTargetNft.uri
                    )}
